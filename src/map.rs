...
pub use crate::inner::entry::{OccupiedEntry, VacantEntry};
...
impl<K, V, S> RingMap<K, V, S> {
    ...
    /// use ringmap::RingMap;
    ///
    /// let mut map: RingMap<i32, i32> = (0..8).map(|x| (x, x)).collect();
    /// let extracted: RingMap<i32, i32> = map.extract_if(.., |k, _v| k % 2 == 0).collect();
    ///
...
impl<K, V, S> RingMap<K, V, S> {
    ...
    /// unspecified and may change between versions of `ringmap` or the standard library.
    ///
